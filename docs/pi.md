...
It is a *seed*, not pi's live config dir, because a read-only mount at the live path fundamentally cannot work: pi wraps even auth **reads** in a file lock (`proper-lockfile` creates `auth.json.lock` next to the file), so on a read-only mount every auth read fails with EROFS and pi reports "No API key found" despite the key being present. Upstream confirmed the lock-on-read is intentional — it is what lets concurrent pi instances coordinate OAuth token refresh on a shared `auth.json` ([earendil-works/pi#6406](https://github.com/earendil-works/pi/issues/6406)) — and token refresh needs real writes regardless. On first run, the container-side provider copies `auth.json` (mode 0600), `models.json`, and `settings.json` from the seed into the writable, per-group `~/.pi/agent` (mode 0700).
