 ...
 * as "No API key found" with the key sitting right there. The lock-on-read
 * is by design upstream — it coordinates OAuth token refresh between
 * concurrent pi instances sharing one auth.json (earendil-works/pi#6406,
 * closed no-action) — and refresh needs real writes anyway. So the
 * writable per-group `~/.pi/agent` is seeded from the RO mount instead.
 *
 ...
 * the --fork/--session fuzzy-resolve paths in `main.js`; accepted upstream
 * as a bug, earendil-works/pi#6407 — this detection can be retired once a
 * fix lands and the pin moves past it). So the provider
