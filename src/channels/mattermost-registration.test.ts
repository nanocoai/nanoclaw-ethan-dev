 ...
 * never at import. It does require the adapter package (`chat-adapter-mattermost`) to be
 * installed, which holds in a composed install: the skill's `pnpm install` step runs
 * before this test — so this test also implicitly guards that dependency (an unmocked
 * import throws if the package is missing).
 *
 * mattermost is a Chat SDK channel: mattermost.ts also consumes a load-bearing *core*
 * API — `createChatSdkBridge(...)` from ./chat-sdk-bridge.js. That core-consumption is a
