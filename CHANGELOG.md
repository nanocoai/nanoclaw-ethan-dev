...
- **`/add-codex` now pins `@openai/codex` 0.149.0** (was 0.146.0, from 2026-07-29). 0.149.0 was released 2026-08-20, the newest release older than three days at pin time; no NanoClaw-side changes are required. Existing codex installs are not re-pinned by re-running `/add-codex` (the manifest merge is keyed on package name): edit the `@openai/codex` entry in `container/cli-tools.json` to `0.149.0`, rebuild the agent image (`./container/build.sh`), and restart.
